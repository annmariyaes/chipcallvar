...
process BCFTOOLS {
    ...
    bcftools +fill-tags ${meta.patient}_with_sb.vcf -Oz -o ${meta.patient}.macs3.vep.filled.vcf.gz -- -t FORMAT/VAF
    tabix -p vcf "${meta.patient}.macs3.vep.filled.vcf.gz"

    # Cleanup
    rm header.txt ${meta.patient}.AD.values.txt.gz ${meta.patient}.AD.values.txt.gz.tbi ${meta.patient}.SB.values.txt.gz ${meta.patient}.SB.values.txt.gz.tbi ${meta.patient}.reheader.vcf ${meta.patient}.with.ad.vcf
   """
}
